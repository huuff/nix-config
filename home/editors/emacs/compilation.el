...
(setq haf/compilation-configs (list '(:marker-file "Cargo.toml" :build-command "cargo build" :mode rust-compilation-mode)
                                    '(:marker-file "pnpm-lock.yaml" :build-command "pnpm check")
                                    '(:marker-file "package-lock.json" :build-command "npm run build")))
...
(defun haf/compilation-config-applies (cfg directory)
  ...
  (file-exists-p (expand-file-name (plist-get cfg :marker-file) directory)))
